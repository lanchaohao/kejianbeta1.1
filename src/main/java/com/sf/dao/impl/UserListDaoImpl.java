...
public class UserListDaoImpl implements UserListDao {
	...
	//查询视频
	public List<videoEntity> Pagevideolist(String State, int dangqianye,int meiyexianshiduoshaoge,int searchType,String key) {
		//SELECT * FROM video WHERE videoID<=5 LIMIT 0,6
		//0为全部视频，1为名字查找，2为标签查找，3为查找，4为综合查找
		System.out.println("type:"+searchType+"|key:"+key);
		String sql="SELECT * FROM video LIMIT ?,?";
		List<videoEntity> list=jdbcTemplate.getJdbcTemplate().query(sql,new Object[]{dangqianye,meiyexianshiduoshaoge} ,new VideoRowMapperEntity());
		if(searchType==1){
			sql="SELECT * FROM video WHERE videoName like ? LIMIT ?,?";
			list=jdbcTemplate.getJdbcTemplate().query(sql,new Object[]{"%"+key+"%",dangqianye,meiyexianshiduoshaoge} ,new VideoRowMapperEntity());
		}
		if(searchType==2){
			sql="SELECT * FROM video WHERE  videoName like ? LIMIT ?,?";
			list=jdbcTemplate.getJdbcTemplate().query(sql,new Object[]{"%"+key+"%",dangqianye,meiyexianshiduoshaoge} ,new VideoRowMapperEntity());
		...
		if(searchType==3){
			sql="SELECT * FROM video WHERE videocAtegory = ? LIMIT ?,?";
		}
	...
	public int videocounByName(String countvideo) {
		//根据标记查询出对应的条数
		String sql="select count(*) from video where videoName like ?";// where videocAtegory = ?
		int num = jdbcTemplate.getJdbcTemplate().queryForObject(sql, new Object[]{"%"+countvideo+"%"},Integer.class);
		return num;
	}
